...
const blockInsert = 'INSERT INTO `blocks` (`hash`, `prevHash`, `height`, `baseReward`, `difficulty`, `majorVersion`, `minorVersion`, `nonce`, `size`, `timestamp`) VALUES (?,?,?,?,?,?,?,?,?,?)'
const blockDelete = 'DELETE FROM `blocks` WHERE `hash` = ?'
const txnInsert = 'INSERT INTO `transactions` (`txnHash`, `blockHash`, `mixin`, `timestamp`, `paymentId`, `unlockTime`, `publicKey`, `fee`, `size`, `nonce`, `extra`) VALUES (?,?,?,?,?,?,?,?,?,?,?)'
const txnDelete = 'DELETE FROM `transactions` WHERE `txnHash` = ?'
const txnInputInsert = 'INSERT INTO `transaction_inputs` (`txnHash`, `keyImage`, `amount`, `type`) VALUES (?,?,?,?)'
const txnInputDelete = 'DELETE FROM `transaction_inputs` WHERE `txnHash` = ?'
const txnOutputInsert = 'INSERT INTO `transaction_outputs` (`txnHash`, `outputIndex`, `globalIndex`, `amount`, `key`, `type`) VALUES (?,?,?,?,?,?)'
const txnOutputDelete = 'DELETE FROM `transaction_outputs` WHERE `txnHash` = ?'
...
Self.prototype.saveBlock = function (block) {
  ...
  /* To make sure we don't have anything weird going on, we need
     remove any pre-existing copy of the block */
  queries.push({ query: blockDelete, args: [block.hash] })
  ...
  for (var i = 0; i < block.transactions.length; i++) {
    ...
    /* To help us resolve any issues caused by resolved chains whereby the transactions
       inside a specific block changed when the block changed we need to clean up any
       remnants of the old transactions */
    queries.push({ query: txnDelete, args: [transaction.hash] })
    queries.push({ query: txnInputDelete, args: [transaction.hash] })
    queries.push({ query: txnOutputDelete, args: [transaction.hash] })
